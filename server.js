...
io.on('connection', (socket) => {
    ...
    setTimeout(function () {
      while (tmp == numClients) {
        for (var client in clients) {
          ...
          console.log(
            'Computenextval from upd sent to ' +
              'Client' +
              nextId +
              ' from Client' +
              currId
          ...
          io.sockets.in('Client' + nextId).emit('ComputeNextYval', {
            Yval: clients[client]['Yval'],
            ret: 0,
            sourceClientId: 'Client' + currId,
            destClientId: 'Client' + nextId,
          });
        }
        tmp = 0;
      }
  ...
  socket.on('sendToNextClient', (data) => {
    ...
    if (ret == numClients - 1) {
      ...
      io.sockets
        .in('Client' + clientNum)
        .emit('AESEncrypt', { Yval: newYval, clientId: clientId });
    } else {
      // else emit next value computation
      var nextId = (clientNum + 1) % numClients;
      console.log(
        'Computenextval sent to ' + 'Client' + nextId + ' from ' + clientId
      );
      io.sockets.in('Client' + nextId).emit('ComputeNextYval', {
        Yval: newYval,
        ret: ret,
